...
(function () {
  ...
  const setActivePanel = (activePanel) => {
    panels.forEach((panelEl) => {
      if (activePanel && panelEl !== activePanel) {
        panelEl.classList.add('inactive');
      } else {
        panelEl.classList.remove('inactive');
      }
    });
  };
  ...
  panels.forEach((panel) => {
    ...
    panel.addEventListener('mouseleave', (event) => {
      ...
      const nextPanel = event?.relatedTarget?.closest?.('.panel');
      if (!nextPanel || !panels.includes(nextPanel)) {
        setActivePanel(null);
      }
    });
